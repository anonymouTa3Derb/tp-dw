...
// ---- 3.4 : Données pour la carte de visualisation ----
// Retourne toutes les stations avec le nombre de POI à proximité et leurs noms
app.get("/api/map-data", async (req, res) => {
    try {
        ...
        const stations = await getDb().collection("velov2026").aggregate([
            ...
            {
                $project: {
                    _id: 0, name: 1, lat: 1, lng: 1, status: 1,
                    pois: 1,
                    poiCount: { $size: "$pois" }
                }
            }
